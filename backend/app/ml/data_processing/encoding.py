...
def encoding(data:pd.DataFrame | None= None):
    ...
    for col in categorial_columns:
            if col in hot_encode_coumns:
                if data is not None:
                   hot_transformed= hot_encode.transform(df[[col]])
                else:
                ...
                if data:
                   ordinal_transformed= ordinal_encoder.transform(df[[col]])
                else:
             ...
             if data is None:
                ...
                joblib.dump(freq_map, f"backend/app/encoders/freq_{col}.pkl")
             else:
                freq_map = joblib.load(f"backend/app/encoders/freq_{col}.pkl")
